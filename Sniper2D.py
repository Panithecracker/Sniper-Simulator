...
def GetX(t,v,theta,L1,mu): #mu is the constant drag coefficient (positive)
    if mu > 0: #some air drag
        x = L1*math.cos(theta)+(v*math.cos(theta))/(mu)*(1-math.exp(-mu*t))
        return x
    x = L1*math.cos(theta)+v*math.cos(theta)*t #no air drag case
    return x
def GetY(t,v,theta,L1,g,mu):
    if mu > 0: #some air drag
        y = L1*math.sin(theta)+((g)/(mu**2)+(v*math.sin(theta))/(mu))*(1-math.exp(-mu*t))-(g)/(mu)*t
        return y
...
register = ax.text(0.05, 0.95, f'Angle : {math.degrees(MyRifle.shooting_angle):.1f}º', transform=ax.transAxes, fontsize=12, verticalalignment='top') #timer and angle counter text box
...
target_x = 30
target_y = 10
target, = ax.plot([target_x],[target_y],marker = '.',markersize = 10)
...
def ShootingAnimation(frame): #shooting animation update function
    ...
    new_x = GetX(t,v,MyRifle.shooting_angle,L1,mu=0.3) #compute the new x and y coordinates
    new_y = GetY(t,v,MyRifle.shooting_angle,L1,g,mu = 0.3)
    bullet.set_data([new_x],[new_y]) #update the data in the bullet plot
    register.set_text(f'Time: {t:.1f} s')  # Update time
    if abs(new_y) > 50 or abs(new_x) > 50: #bullet out of the x,y limits so dont animate anymore
        ...
        register.set_text(f'Angle : {math.degrees(MyRifle.shooting_angle):.1f}º')  # Display angle info again
    ...
    if frame == math.ceil(duration/dt): #last frame so update bullet state
        in_air = False
        register.set_text(f'Angle : {math.degrees(MyRifle.shooting_angle):.1f}º')  # Display angle info again
    return bullet, register #return anything
...
def on_key_press(event): #key_press event handler
    ...
    if key == 'up' and not in_air and not rotating: #only animate up when no other animation is going on (neither rotation nor bullet shot)
        ...
        register.set_text(f'Angle : {math.degrees(MyRifle.shooting_angle):.1f}º')  # Update time text and angle info
        rotating = False
